...
{
	// Sensor angle relative to the trace segment
	...
	{
	case 3:
		sensor_out = sensor_calc_str(car->_trkPos.seg, car->_trkPos.toLeft, car->_trkPos.toStart, -relative_sensor_angle, sensor_range);
		break;
	case 2:
		sensor_out = sensor_calc_lft_rgt(car->_trkPos.seg, car->_trkPos.toLeft, car->_trkPos.toStart, -relative_sensor_angle, sensor_range);
		break;
	case 1:
		sensor_out = sensor_calc_lft_rgt(car->_trkPos.seg, car->_trkPos.toLeft, car->_trkPos.toStart, -relative_sensor_angle, sensor_range);
		break;
...
float SingleSensor::sensor_calc_str(tTrackSeg *seg, float x_coord, float y_coord, float angle, float remaining_range)
{
	...
	{
		if ((x_sx / y_up) > tan(angle)) // Might meet the top edge, check the range
		{
			float partial = y_up / cos(angle);
			if (partial >= remaining_range)
				return remaining_range;
			else
			{
				...
				{
				case 3:
					partial_returned = sensor_calc_str(seg->next, x, 0, angle, (remaining_range - partial));
					break;
				case 2:
					partial_returned = sensor_calc_lft_rgt(seg->next, x, 0, angle, (remaining_range - partial));
					break;
				case 1:
					partial_returned = sensor_calc_lft_rgt(seg->next, x, 0, angle, (remaining_range - partial));
					break;
		...
		else // Might meet the left edge, check the range
		{
			float temp_val = x_sx / cos((HALF_PI)-angle);
			if (temp_val >= remaining_range)
				return remaining_range;
			else
				return temp_val;
		}
	...
	{
		if ((x_dx / y_up) > -tan(angle)) // Might meet the top edge, check the range
		{
			float partial = y_up / cos(angle);
			if (partial >= remaining_range)
				return remaining_range;
		...
		else // Might meet the right edge, check the range
		{
	...
	{
		if ((x_sx / y_dn) > tan(PI - angle)) // Might meet the bottom edge, check the range
		{
			...
			else
			{
				...
				{
				case 3:
					partial_returned = sensor_calc_str(seg->prev, x, seg->prev->length, angle, (remaining_range - partial));
					break;
				case 2:
					partial_returned = sensor_calc_lft_rgt(seg->prev, x, seg->prev->arc, angle, (remaining_range - partial));
					break;
				case 1:
					partial_returned = sensor_calc_lft_rgt(seg->prev, x, seg->prev->arc, angle, (remaining_range - partial));
					break;
	...
	{
		if ((x_dx / y_dn) > tan(PI + angle)) // Might meet the bottom edge, check the range
		{
...
float SingleSensor::sensor_calc_lft_rgt(tTrackSeg *seg, float x_coord, float y_coord, float angle, float remaining_range)
{
	...
	float x_returned;
	if (check_max_circle_intersect(temp_angle, radius_max, radius_x, y_dn, y_up, remaining_range, dist_returned))
		return dist_returned;
	else if (check_min_circle_intersect(temp_angle, radius_min, radius_x, y_dn, y_up, remaining_range, dist_returned))
		return dist_returned;
